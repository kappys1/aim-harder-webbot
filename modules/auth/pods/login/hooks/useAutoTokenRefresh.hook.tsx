...
export const useAutoTokenRefresh = ({
  ...
  const actualIntervalMinutes =
    intervalMinutes ?? ENV.getFrontendRefreshIntervalMinutes();
    ...
    const fetchCurrentToken = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `/api/auth/refresh?email=${encodeURIComponent(userEmail)}`,
          {
            method: "GET",
            headers: {
        ...
        if (response.ok) {
          ...
          if (data.success && data.token) {
            ...
            console.log(
              "Current token fetched for auto-refresh:",
  ...
  const startAutoRefresh = useCallback(() => {
    ...
    if (!enabled || !userEmail || !currentToken) {
      console.log("Auto token refresh not started:", {
        enabled,
        hasUser: !!userEmail,
        hasToken: !!currentToken,
      });
    ...
    console.log(
      `Starting auto token refresh every ${actualIntervalMinutes} minutes for user:`,
      userEmail
    );
    ...
    console.log(actualIntervalMinutes * 60 * 1000);
