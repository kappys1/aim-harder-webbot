...
class AuthService {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 429 && data.rateLimited) {
          return {
            success: false,
            error: data.error || `Too many attempts. Try again in ${data.minutesRemaining} minutes.`,
          }
        }
  ...
  async checkSession(email: string): Promise<{
    ...
    try {
      const response = await fetch(`/api/auth/aimharder?email=${encodeURIComponent(email)}`, {
        method: 'GET'
      })
