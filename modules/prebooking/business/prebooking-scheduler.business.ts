...
export class PreBookingScheduler {
   ...
   */
  async execute(): Promise<{success: boolean; message: string; details: any}> {
    ...
    try {
      ...
      console.log(`[PreBookingScheduler] Querying prebookings between ${startRange.toISOString()} and ${endRange.toISOString()}`);

      const pendingPrebookings = await preBookingService.findPendingInTimeRange(startRange, endRange);
      ...
      console.log(`[PreBookingScheduler] Found ${pendingPrebookings.length} pending prebooking(s)`);
   ...
   */
  private async loadPrebookingsIntoMemory(prebookings: PreBooking[]): Promise<void> {
    console.log(`[PreBookingScheduler] Loading ${prebookings.length} prebooking(s) into memory...`);
    ...
    for (const prebooking of prebookings) {
      try {
        ...
        if (!session) {
          console.error(`[PreBookingScheduler] No session found for user ${prebooking.userEmail}`);
        ...
        console.log(`[PreBookingScheduler] Loaded prebooking ${prebooking.id} for ${prebooking.userEmail}`);

      } catch (error) {
        console.error(`[PreBookingScheduler] Error loading prebooking ${prebooking.id}:`, error);
        await preBookingService.updateStatus({
          id: prebooking.id,
          status: 'failed',
          errorMessage: error instanceof Error ? error.message : 'Loading error',
          executedAt: new Date(),
        });
      }
    }

    console.log(`[PreBookingScheduler] Loaded bookings grouped by timestamp: ${this.loadedBookings.size} group(s)`);
  }
  ...
  private async waitAndExecute(startTime: number): Promise<void> {
    ...
    console.log('[PreBookingScheduler] Starting interval to wait for execution time...');
    ...
    return new Promise((resolve, reject) => {
      const checkInterval = setInterval(async () => {
        ...
        if (elapsed > this.MAX_EXECUTION_TIME) {
          console.error('[PreBookingScheduler] Max execution time exceeded, stopping');
        ...
        for (const timestamp of this.loadedBookings.keys()) {
          if (now >= timestamp) {
            ...
            console.log(`[PreBookingScheduler] Waiting ${secondsUntil}s until next execution...`);
          }
        }
        ...
        for (const timestamp of timestampsToExecute) {
          ...
          console.log(`[PreBookingScheduler] Executing ${bookings.length} booking(s) at timestamp ${new Date(timestamp).toISOString()}`);
        ...
        if (this.loadedBookings.size === 0) {
          console.log('[PreBookingScheduler] All prebookings executed, stopping interval');
          clearInterval(checkInterval);
          resolve();
        }
   ...
   */
  private async executePrebookingsFIFO(loadedBookings: LoadedPreBooking[]): Promise<void> {
   ...
   */
  private async executeOne(prebooking: PreBooking, cookies: AuthCookie[]): Promise<void> {
    console.log(`[PreBookingScheduler] Executing prebooking ${prebooking.id} for ${prebooking.userEmail}...`);
    ...
    try {
      ...
      const bookingResponse = await bookingService.createBooking(prebooking.bookingData, cookies);
      const executionTime = Date.now() - executionStart;

      console.log(`[PreBookingScheduler] Booking execution took ${executionTime}ms - bookState: ${bookingResponse.bookState}`);

      // Update with result
      if (bookingResponse.bookState === 1) {
        // Success
        await preBookingService.updateStatus({
          id: prebooking.id,
          status: 'completed',
        ...
        console.log(`[PreBookingScheduler] ✅ Prebooking ${prebooking.id} completed successfully`);
        ...
        console.log(`[PreBookingScheduler] ❌ Prebooking ${prebooking.id} failed - bookState: ${bookingResponse.bookState}`);
      }

    } catch (error) {
      console.error(`[PreBookingScheduler] Error executing prebooking ${prebooking.id}:`, error);
      await preBookingService.updateStatus({
        id: prebooking.id,
        status: 'failed',
        executedAt: new Date(),
  ...
  getStats() {
    return {
      loadedBookingsCount: Array.from(this.loadedBookings.values()).reduce((sum, arr) => sum + arr.length, 0),
      activeIntervalsCount: this.activeIntervals.size,
      timestamps: Array.from(this.loadedBookings.keys()).map(ts => new Date(ts).toISOString()),
    };
  }
}
